...
namespace Moletrator.SQLDocumentor
{
	...
	public class SQLInfoEnumerator
	{
		...
		/// <returns></returns>
		public string[] RetrieveInformation(string InputParam)
		{
			...
			{			
				throw new ApplicationException("Cannot Locate SQL Server." + ex.Message);
			}
			finally
			{
        ...
        public static string BuildConnectionstring(string server, string database, string user, string password, bool integratedSecurity)
        {
        ...
        public static bool TestConnection(string server, string database, string user, string password, bool integratedSecurity, out  object[] databases)
        {
            
            string connectionString = BuildConnectionstring(server, database, user, password, integratedSecurity);
            return TestConnection(connectionString,out databases);
        }

        public static bool TestConnection(string connectionString,out object[] databases)
        {
            ArrayList dbarraylist = new ArrayList();
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    conn.Open();
                    SqlCommand command = new SqlCommand("select name from sys.databases", conn) { CommandType = CommandType.Text };

                    SqlDataReader reader = command.ExecuteReader();
                    int nameIndex = reader.GetOrdinal("name");
                    while (reader.Read())
                    {
                        dbarraylist.Add(reader.GetString(nameIndex));
                    }
                    databases = dbarraylist.ToArray();
        ...
        public static bool TestConnectionMySQL(string connectionStringMSSQL,string connectionStringMYSQL,string database,out  object[] databases)
        {
            
            
            string query = @"IF Not EXISTS (SELECT srv.name FROM sys.servers srv WHERE srv.server_id != 0 AND srv.name = N'MYSQL')
BEGIN
   EXEC master.dbo.sp_addlinkedserver 
   @server = N'MYSQL', 
   @srvproduct=N'MySQL', 
   @provider=N'MSDASQL', 
   @provstr=N'"+connectionStringMYSQL+@"'
    print 'added'
END
ELSE IF EXISTS (SELECT srv.name FROM sys.servers srv WHERE srv.server_id != 0 AND srv.name = N'MYSQL' and 
provider_string <> '" + connectionStringMYSQL + @"')
BEGIN
    ...
    print 'edited'
End
--need to use sp_executesql so that it will not execute first
exec sp_executesql N'select  [schema_name] from openquery(MYSQL,''select schema_name from information_schema.SCHEMATA'')'";
            ArrayList dbarraylist = new ArrayList();
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand command = new SqlCommand(query, conn) { CommandType = CommandType.Text };

                    SqlDataReader reader = command.ExecuteReader();
                    int nameIndex = reader.GetOrdinal("schema_name");
                    while (reader.Read())
                    {
                        dbarraylist.Add(reader.GetString(nameIndex));
                    }
